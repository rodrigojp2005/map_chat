...
<script>
    ...
    function initMap() {
        ...
        map = new google.maps.Map(document.getElementById('map-criar'), { center: defaultLocation, zoom: 12 });
        geocoder = new google.maps.Geocoder();
        streetView = new google.maps.StreetViewPanorama(
            document.getElementById('street-view'), { position: defaultLocation, pov: { heading: 165, pitch: 0 }, zoom: 1 }
        );
        marker = new google.maps.Marker({ position: defaultLocation, map, draggable: true });
        ...
        marker.addListener('dragend', function() { updateLatLngFields(marker.getPosition()); atualizarStreetView(marker.getPosition()); });
        map.addListener('click', function(event) { marker.setPosition(event.latLng); updateLatLngFields(event.latLng); atualizarStreetView(event.latLng); });
    }

    function atualizarStreetView(position) { streetView.setPosition(position); }

    let buscarCidadeTimeout; function debouncedBuscarCidade() { clearTimeout(buscarCidadeTimeout); buscarCidadeTimeout = setTimeout(buscarCidade, 500); }
    function buscarCidade() {
        ...
        geocoder.geocode({ address: endereco }, function (results, status) {
            if (status === 'OK') {
                const location = results[0].geometry.location; map.panTo(location); map.setZoom(16); marker.setPosition(location);
                document.getElementById('latitude').value = location.lat(); document.getElementById('longitude').value = location.lng();
                feedback.textContent = 'Endereço encontrado: ' + results[0].formatted_address; feedback.style.color = '#198754'; atualizarStreetView(location);
